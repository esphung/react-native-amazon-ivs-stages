...
export const transformRTMPSUrl = (
...
const useStore = () => {
  ...
  const [stageHostDetails, setStageHostDetails] =
    React.useState<StageHostDetails>();
  const [stageJoinDetails, setStageJoinDetails] =
    React.useState<StageJoinDetails>();

  console.debug(
    'stageDetailsState.stages',
    stageDetailsState.stages.map((s) => s.groupId)
  );
  ...
  const onSelectStageDetails = React.useCallback(
    (stageDetails: StageDetails | null) => {
      setStageDetailsState((prev) => {
        if (
          prev.selected?.channelId !== stageDetails?.channelId &&
          stageDetails?.channelId
        ) {
          return {
            ...prev,
  ...
  const createNewUser = React.useCallback(
    (inputValues: { username: string; avatarUrl: string }) => {
      setCurrentUser({
        userId: uuid.v4(),
  ...
  const updateUser = React.useCallback(({ isHost }: Pick<IUser, 'isHost'>) => {
    setCurrentUser((prev) => {
      if (!prev) {
        return {
          userId: uuid.v4(),
  ...
  const onPressSignIn = React.useCallback(
    ({ username, avatarUrl }: Pick<IUser, 'username' | 'avatarUrl'>) => {
      createNewUser({ username, avatarUrl });
      showStageList();
    },
    // eslint-disable-next-line react-hooks/exhaustive-deps
    []
  ...
  const refreshCurrentStageDetailsList = React.useCallback(() => {
    setStageDetailsState((prev) => ({ ...prev, loading: true }));
    getAllStages()
      .then((res) => {
        setStageDetailsState((prev) => ({
          ...prev,
      ...
      .then(() =>
        setStageDetailsState((prev) => ({ ...prev, loading: false }))
      );
  }, []);
