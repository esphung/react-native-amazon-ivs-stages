...
export type ExtractComponentProps<T> = T extends ComponentType<infer P> | Component<infer P> ? P : never;
...
export declare enum NetworkHealthEnum {
    EXCELLENT = 0,
    HIGH = 1,
    MEDIUM = 2,
    LOW = 3,
...
export declare enum BroadcastQualityEnum {
    NEAR_MAXIMUM = 0,
    HIGH = 1,
    MEDIUM = 2,
    LOW = 3,
...
type AudioSessionStrategy = 'recordOnly' | 'playAndRecord' | 'playAndRecordDefaultToSpeaker' | 'noAction';
type ConfigurationPreset = 'standardPortrait' | 'standardLandscape' | 'basicPortrait' | 'basicLandscape';
...
export interface INativeEventHandlers {
    onError: IEventHandler<Readonly<{
        message: string;
    }>>;
    onBroadcastError: IEventHandler<Readonly<{
        exception: {
    ...
    onIsBroadcastReady: IEventHandler<Readonly<{
        isReady: boolean;
    }>>;
    onBroadcastAudioStats: IEventHandler<Readonly<{
        audioStats: IAudioStats;
    }>>;
    onBroadcastStateChanged: IEventHandler<Readonly<{
     ...
     */
    onBroadcastQualityChanged: IEventHandler<Readonly<{
        quality: number;
    }>>;
    /**
     * @deprecated in favor of onTransmissionStatisticsChanged
     */
    onNetworkHealthChanged: IEventHandler<Readonly<{
        networkHealth: number;
    }>>;
    onTransmissionStatisticsChanged: IEventHandler<Readonly<{
        statistics: INativeTransmissionStatistics;
    }>>;
...
export interface IIVSBroadcastCameraNativeViewProps extends IBaseProps, INativeEventHandlers {
...
export interface IIVSBroadcastCameraViewProps extends IBaseProps, IEventHandlers {
    readonly style?: StyleProp<ViewStyle>;
    readonly testID?: string;
}
...
export {};
//# sourceMappingURL=IVSBroadcastCameraView.types.d.ts.map